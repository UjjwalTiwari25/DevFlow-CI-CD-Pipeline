...
import { joinCourse } from '@/models/course';
...
export default function useCoursePayment({
  amount,
  course,
  community,
  stripe,
  ...
  useEffect(() => {
    dispatchRedux(setCourse(course));
  }, [course]);

  const processCoursePayment = useCallback(async () => {
    ...
    if (!response || response.error) {
      ...
      Toast.showError('Failed to process course payment');
      return;
    }
    onSuccessfulPurchase();
  }, [
  ...
  const handleProcessCommunitySubscription = useCallback(
    async ({ tokenData }) => {
      try {
        ...
        onSubscriptionPollFail();
        Toast.showError(
          'Your community subscription has been applied but course purchase failed. Please try again to purchase the course.'
        );
        return false;
      } catch (error) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (coursePaymentType === 'communityWithCourse') {
        await handleProcessCommunitySubscription({
          ...
          if (course.communityPrice > 0) {
            await processCoursePayment({
              paymentMethodType: paymentConstants.PAYMENT_METHOD_CARD,
              tokenData: authorization,
            });
          } else {
            await joinCourse(course.id)
              ...
              .catch((error) => {
                notifyHandledError(error, { message: 'Failed to join course' });
                Toast.showError('Failed to join course');
              });
          }
        });
      } else if (coursePaymentType === 'community') {
        await handleProcessCommunitySubscription({
          paymentMethodType: paymentConstants.PAYMENT_METHOD_CARD,
          tokenData: authorization,
      ...
      } else if (coursePaymentType === 'course') {
        await processCoursePayment({
          paymentMethodType: paymentConstants.PAYMENT_METHOD_CARD,
          tokenData: authorization,
        });
