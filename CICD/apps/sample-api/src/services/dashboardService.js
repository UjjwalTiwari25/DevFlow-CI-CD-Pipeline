...
async function getStats(userId) {
  const repos = await prisma.repository.findMany({ where: { ownerId: userId }, select: { id: true } });
  const repoIds = repos.map((r) => r.id);
  if (!repoIds.length) return { totalRuns: 0, successRate: 0, avgDuration: 0, deploysToday: 0, totalRepos: 0, activeScans: 0 };
  ...
  const [totalRuns, successRuns, allRuns, deploysToday] = await Promise.all([
    ...
    prisma.pipelineRun.findMany({ where: { repoId: { in: repoIds }, duration: { not: null } }, select: { duration: true } }),
    prisma.deployment.count({
      where: { repoId: { in: repoIds }, createdAt: { gte: new Date(new Date().setHours(0, 0, 0, 0)) } },
    }),
  ]);

  const avgDuration = allRuns.length ? Math.round(allRuns.reduce((s, r) => s + r.duration, 0) / allRuns.length) : 0;
  const successRate = totalRuns ? Math.round((successRuns / totalRuns) * 1000) / 10 : 0;

  return { totalRuns, successRate, avgDuration, deploysToday, totalRepos: repoIds.length, activeScans: repoIds.length * 5 };
}
...
async function getRepositories(userId) {
  return prisma.repository.findMany({
    ...
    include: {
      ...
      pipelines: { orderBy: { startedAt: 'desc' }, take: 1, select: { status: true, startedAt: true } },
    },
...
async function getPipelines(userId, query = {}) {
  ...
  return { pipelines, pagination: { page: Number(page), limit: Number(limit), total, totalPages: Math.ceil(total / limit) } };
}
...
async function rerunPipeline(pipelineId) {
  ...
  return prisma.pipelineRun.create({
    data: {
      commitMsg: `Re-run: ${original.commitMsg}`, commitSha: original.commitSha,
      branch: original.branch, trigger: 'manual', repoId: original.repoId, status: 'QUEUED',
    },
  });
}
...
async function getDeployments(userId, query = {}) {
  ...
  return { deployments, pagination: { page: Number(page), limit: Number(limit), total, totalPages: Math.ceil(total / limit) } };
}

async function triggerDeployment(repoId) {
  const repo = await prisma.repository.findUnique({ where: { id: repoId } });
  if (!repo) throw new Error('Repository not found');
  const lastDeploy = await prisma.deployment.findFirst({ where: { repoId }, orderBy: { createdAt: 'desc' } });
  const parts = (lastDeploy?.version || 'v1.0.0').replace('v', '').split('.').map(Number);
  parts[2]++;
  const version = `v${parts.join('.')}`;
  return prisma.deployment.create({
    data: { version, commitSha: Math.random().toString(16).slice(2, 9), environment: 'production', status: 'DEPLOYING', triggeredBy: 'manual', repoId },
  });
}
...
async function getSecurityScans(userId, query = {}) {
  ...
  return { scans, pagination: { page: Number(page), limit: Number(limit), total, totalPages: Math.ceil(total / limit) } };
}
...
async function getHealthStatus() {
  ...
  return {
    status: 'healthy', uptime: process.uptime(), dbLatency,
    memory: { used: Math.round(mem.heapUsed / 1024 / 1024), total: Math.round(mem.heapTotal / 1024 / 1024), rss: Math.round(mem.rss / 1024 / 1024) },
    version: process.env.npm_package_version || '1.0.0', nodeVersion: process.version, timestamp: new Date().toISOString(),
  };
}

module.exports = {
  getStats, getRepositories, createRepository, deleteRepository,
  getPipelines, getPipelineById, rerunPipeline,
  getDeployments, triggerDeployment, rollbackDeployment,
  getSecurityScans, getHealthStatus,
};
